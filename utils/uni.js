 ...
 * @param {*} options
 */
export function uniLoading(content, {
  isMask = true,
  ...moreOptions
} = {}) {
  return {
 ...
 * @param {*} options
 */
export async function uniAuthorize(scope, {
  tips = '',
} = {}) {
      ...
      ? {
        longPressActions: {
          ...
          success({
            ...
            actionName[data.tapIndex].success && actionName[data.tapIndex].success({
              item: list[index],
 ...
 * @desc 调起内置地图选择位置
 */
export function uniChooseLocation({
  ...moreOptions
} = {}) {
  return new Promise((resolve) => {
    uni.chooseLocation({
      ...
      uni.getNetworkType({
        success(res) {
          if (res.networkType == 'none') {
            ...
            if (currentPage == '/pages/subPages/offline/index') {
              uniToast('没有网络')
            } else {
              uni.navigateTo({
                url: '/pages/subPages/offline/index',
              })
              app.globalData.pageOptions = to
            }
          } else if (!to) {
            uni.redirectTo(app.globalData.pageOptions)
            app.globalData.pageOptions = undefined
          } else {
            uni.navigateTo(to)
          }
          resolve()
